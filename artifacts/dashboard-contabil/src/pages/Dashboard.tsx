...
function StatusBadge({ status }: { status: string }) {
  ...
  if (status === 'NÃO OBRIGADA')
    return <span className="badge-blue">{status}</span>;
  if (status === 'SEM MOVIMENTO')
...
export default function Dashboard() {
  ...
  function openDrill(filter: 'all' | 'dia' | 'at', cols?: { key: keyof EmpresaRow; label: string }[]) {
  ...
  function byResp(resp: string) {
    return empresas.filter(e => !resp || e.responsavel === resp);
  }

  const respCounts = countBy(empresas, 'responsavel');
  const integCounts = countBy(empresas, 'integracaoFinanceira');
  const regimeCounts = countBy(empresas, 'regimeTributario');
  const ativCounts = countBy(empresas, 'atividadeOperacional');
  const folhaCounts = countBy(empresas, 'folhaPagamento');
  const statusCounts = countBy(empresas, 'statusFechamento');
  const compCounts = countBy(empresas, 'competenciaFechamento');

  const fechMesData = MESES.map(mes => {
    ...
    return { name: mes, total: fechadas.length, emDia: dia, emAtraso: at };
  });
  ...
  async function gerarPDF() {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(13);
    doc.text('RESUMO GERAL DE FECHAMENTOS', 14, y);
    y += 8;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.text('FECHAMENTOS POR COMPETÊNCIA (MÊS)', 14, y);
    y += 6;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.text('COMPETÊNCIA DE FECHAMENTO (COLUNA K)', 14, y);
    y += 6;
  ...
  function GenericSection({
    title, subtitle, field, label, rows, extraCols
  }: {
      ...
      <div>
        ...
        <div className="chart-card">
          <div className="chart-card-header">
            <div>
              <div className="chart-card-title"><span className="dot-orange" />{label}</div>
            </div>
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={chartData} layout="vertical" margin={{ left: 20, right: 30 }}>
              ...
              <Bar dataKey="value" fill={ORANGE} radius={[0, 4, 4, 0]} cursor="pointer"
                onClick={d => openDrillByField(field, d.name)}>
    ...
    <div className="dashboard">
      ...
      <main className="main-content">
        ...
        {section === 'home' && (
          <div className="fade-in">
            ...
            <div className="section-sub">
              ...
              {responsaveis.map(r => ` · ${r}: ${byResp(r).length}`).join('')}
            </div>
            ...
            <div className="panel-row">
              <div className="comp-card">
                ...
                <table className="comp-table">
                  <thead>
                    <tr>
                      <th>Competência</th><th>Fechadas</th><th>Pendentes</th>
                      <th>Em Dia</th><th>Em Atraso</th><th>% Fechado</th><th>Progresso</th>
                    </tr>
                  </thead>
                  <tbody>
              ...
              <div className="comp-card">
                ...
                <table className="comp-table">
                  <thead>
                    <tr>
                      <th>Competência</th><th>Total</th><th>Em Dia</th><th>Em Atraso</th><th>% Total</th><th>Progresso</th>
                    </tr>
                  </thead>
                  <tbody>
        ...
        {section === 'empresas' && (() => {
          const [resp, setResp] = useState('');
          const [s, setS] = useState('');
          const counts = countBy(empresas.filter(e => !resp || e.responsavel === resp), 'responsavel');
          ...
          const statusChartData = responsaveis.map(r => {
            ...
            return { name: r, 'Em Dia': sub.filter(e => e.statusFechamento === 'EM DIA').length, 'Em Atraso': sub.filter(e => e.statusFechamento === 'EM ATRASO').length };
          });
          ...
          return (
            <div className="fade-in">
              ...
              <div className="section-sub">{total} empresas na base · ordem alfabética</div>
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={chartData}>
                      ...
                      <Bar dataKey="value" radius={[4, 4, 0, 0]} cursor="pointer"
                        onClick={d => openDrillByField('responsavel', d.name)}>
              ...
              <div className="table-card">
                ...
                <div className="table-wrap">
                  <table className="data-table">
                    <thead>
                      <tr>
                        <th>CNPJ</th><th>Empresa</th><th>Cód.</th><th>Integração</th>
                        <th>Atividade</th><th>Regime</th><th>Folha</th><th>Responsável</th><th>Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {filtered.map((row, i) => (
        ...
        {/* ══════════ INTEGRAÇÃO ══════════ */}
        {section === 'integ' && (
          <GenericSection title="🔗 Integração Financeira" subtitle="Distribuição por sistema de integração"
            field="integracaoFinanceira" label="Empresas por Integração Financeira" rows={empresas}
            extraCols={[{ key: 'integracaoFinanceira', label: 'Integração Financeira' }]} />
        )}
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 30 }}>
                      ...
                      <Bar dataKey="value" radius={[0, 4, 4, 0]} cursor="pointer"
                        onClick={d => openDrillByField('regimeTributario', d.name)}>
                        <LabelList dataKey="value" position="right" style={{ fontSize: 11, fontWeight: 700 }} />
                        {chartData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                      </Bar>
                    </BarChart>
        ...
        {/* ══════════ ATIVIDADE ══════════ */}
        {section === 'ativ' && (
          <GenericSection title="⚙️ Atividade Operacional" subtitle="Distribuição por atividade"
            field="atividadeOperacional" label="Empresas por Atividade Operacional" rows={empresas}
            extraCols={[{ key: 'atividadeOperacional', label: 'Atividade Operacional' }]} />
        )}
                ...
                <div className="chart-card">
                  ...
                  <ResponsiveContainer width="100%" height={220}>
                    <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 30 }}>
                      ...
                      <Tooltip />
                      <Bar dataKey="value" radius={[0, 4, 4, 0]} cursor="pointer"
                        onClick={d => openDrillByField('folhaPagamento', d.name)}>
                        <LabelList dataKey="value" position="right" style={{ fontSize: 11, fontWeight: 700 }} />
                        {chartData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                      </Bar>
                    </BarChart>
        ...
        {section === 'status' && (() => {
          ...
          return (
            <div className="fade-in">
              ...
              <div className="section-sub">{emDia} Em Dia · {emAtraso} Em Atraso · {total} Total</div>
              <div className="chart-grid">
                <div className="chart-card">
                  <div className="chart-card-header">
            ...
            <div className="fade-in">
              ...
              <div className="chart-card" style={{ marginBottom: 16 }}>
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <BarChart data={fechMesData.map(d => ({
                    ...d,
                    emDia: empresas.filter(e => {
                      const key = `fech_${d.name.replace('.', '_')}` as keyof EmpresaRow;
                      return e[key] && (!resp || e.responsavel === resp) && (!st || e.statusFechamento === 'EM DIA');
                    }).length,
                    emAtraso: empresas.filter(e => {
                      const key = `fech_${d.name.replace('.', '_')}` as keyof EmpresaRow;
                      return e[key] && (!resp || e.responsavel === resp) && (!st || e.statusFechamento === 'EM ATRASO');
                    }).length,
                  }))}>
                    <CartesianGrid strokeDasharray="3 3" vertical={false} />
                    <XAxis dataKey="name" tick={{ fontSize: 11 }} />
        ...
        {section === 'obrig' && (
          <div className="fade-in">
            ...
            {obrigTab === 'ecd' && (() => {
              const [resp, setResp] = useState('');
              const [s, setS] = useState('');
              const ecdData = data.ecdEcf;
              const filteredEcd = ecdData.filter(e => {
                if (resp && e.responsavel !== resp) return false;
                if (s && !e.empresa.toLowerCase().includes(s.toLowerCase())) return false;
                return true;
              });
              const ecdCounts = countBy(ecdData, 'ecd');
              const ecfCounts = countBy(ecdData, 'ecf');
              const ecdChart = toChartData(ecdCounts);
              const ecfChart = toChartData(ecfCounts);
              return (
                <div>
                  <div className="chart-grid">
                    <div className="chart-card">
                  ...
                  <div className="table-card">
                    <div className="table-header">
                      ...
                      <div className="table-filters">
                        <select className="filter-select-dark" value={resp} onChange={e => setResp(e.target.value)}>
                          <option value="">Todos os Responsáveis</option>
                          {[...new Set(ecdData.map(e => e.responsavel))].map(r => <option key={r}>{r}</option>)}
                        </select>
                    ...
                    <div className="table-footer">{filteredEcd.length} registros</div>
                  </div>
                </div>
              );
            })()}
            ...
            {obrigTab === 'reinf' && (() => {
              const [resp, setResp] = useState('');
              const [s, setS] = useState('');
              const reinfData = data.reinf;
              const filtered = reinfData.filter(e => {
                if (resp && e.responsavel !== resp) return false;
                if (s && !e.empresa.toLowerCase().includes(s.toLowerCase())) return false;
                return true;
              });
              const trimData = ['t1', 't2', 't3', 't4'].map((t, i) => ({
                ...
                SIM: reinfData.filter(e => (e as any)[t] === 'SIM').length,
                NÃO: reinfData.filter(e => (e as any)[t] === 'NÃO').length,
              }));
              return (
                <div>
                  <div className="chart-card" style={{ marginBottom: 16 }}>
                  ...
                  <div className="table-card">
                    <div className="table-header">
                      ...
                      <div className="table-filters">
                        <select className="filter-select-dark" value={resp} onChange={e => setResp(e.target.value)}>
                          <option value="">Todos os Responsáveis</option>
                          {[...new Set(reinfData.map(e => e.responsavel))].map(r => <option key={r}>{r}</option>)}
                        </select>
                        <input className="filter-input" placeholder="Buscar..." value={s} onChange={e => setS(e.target.value)} />
                      </div>
                    </div>
            ...
            {obrigTab === 'impl' && (() => {
              const [resp, setResp] = useState('');
              const [s, setS] = useState('');
              const implData = data.implementacoes;
              const filtered = implData.filter(e => {
                if (resp && e.responsavel !== resp) return false;
                if (s && !e.empresa.toLowerCase().includes(s.toLowerCase())) return false;
                return true;
              });
              const implChart = [
                { name: 'ONVIO', SIM: implData.filter(e => e.onvio === 'SIM').length, NÃO: implData.filter(e => e.onvio === 'NÃO').length },
                { name: 'NIBO', SIM: implData.filter(e => e.nibo === 'SIM').length, NÃO: implData.filter(e => e.nibo === 'NÃO').length },
                { name: 'FISCAL/FOLHA', SIM: implData.filter(e => e.fiscalFolha === 'SIM').length, NÃO: implData.filter(e => e.fiscalFolha === 'NÃO').length },
              ];
              return (
                <div>
                  <div className="chart-card" style={{ marginBottom: 16 }}>
                  ...
                  <div className="table-card">
                    <div className="table-header">
                      ...
                      <div className="table-filters">
                        <select className="filter-select-dark" value={resp} onChange={e => setResp(e.target.value)}>
                          <option value="">Todos os Responsáveis</option>
                          {[...new Set(implData.map(e => e.responsavel))].map(r => <option key={r}>{r}</option>)}
                        </select>
                        <input className="filter-input" placeholder="Buscar..." value={s} onChange={e => setS(e.target.value)} />
                      </div>
                    </div>
